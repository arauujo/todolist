...
import br.com.wesleyaraujo.todolist.utils.Utils;
...
public class TaskController {
    ...
    // http://localhost:8080/tasks/5bf8fc04-5b0f-440a-8834-10936bb33507
    @PutMapping("/{id}")
    public ResponseEntity update(@RequestBody TaskModel taskModel, @PathVariable UUID id, HttpServletRequest request) {
        ...
        if (!task.getIdUser().equals(idUser)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body("Essa tarefa não pertence ao usuário que está logado!");
        }

        Utils.copyNonNullProperties(taskModel, task);

        var taskUpdated = this.taskReposity.save(task);
